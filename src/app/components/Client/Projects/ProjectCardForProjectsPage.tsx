...
const ProjectCardForProjectsPage = ({ article, dark = false, index }: Props) => {
	const slide = {
		initial: {
			scaleY: '100%',
		},
		...
		<article className='w-full odd:ml-auto '>
			<Link href={`/work${article.link}`} className='group flex flex-col gap-5 pb-2 focus-visible:outline-none'>
				<div className='relative aspect-[1.14840989399 after:absolute after:inset-0 after:top-0 after:left-0 after:z-10 after:bg-transparent after:transition-colors hover:after:bg-ThemePrimary/40 group-focus-visible:after:bg-ThemePrimary/40'>
					...
					<Image src={article.card_Image} width={750} height={750} alt='image' className='image' />
				</div>
				...
				<div className={`flex flex-col gap-2 3xl:gap-4 ${dark ? 'text-white' : 'text-ThemeSecondary 3xl:font-medium'}`}>
					<h2 className='group-hover:text-ThemePrimary group-focus-visible:text-ThemePrimary transition-colors font-extrabold text-3xl lg:text-4xl 3xl:text-5xl 5xl:text-[3.5rem]'>
						{article.title}
					</h2>
